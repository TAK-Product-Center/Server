...
angular.module('roger_federation.Workflows')
    .controller('ConnectionsController', ['$rootScope', '$scope', '$http', '$stateParams', '$modalInstance', '$timeout', '$log', '$cookieStore', 'growl', 'WorkflowTemplate', 'WorkflowService', 'JointPaper', connectionsController]);

function connectionsController($rootScope, $scope, $http, $stateParams, $modalInstance, $timeout, $log, $cookieStore, growl, WorkflowTemplate, WorkflowService, JointPaper) {
    ...
    function pollDataFlows() {
        WorkflowService.getFedhubMetrics().then(function(metrics) {
            $scope.flows = metrics.channelInfos;
          });
    ...
    function getCaGroups() {
        WorkflowService.getCaGroupNames().then(
            function(caList) {
