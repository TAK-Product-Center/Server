...
.controller('WorkflowsController', ['$scope', '$rootScope', '$window', '$state', '$stateParams', '$interval', '$timeout', '$uibModal',
  '$log', '$http', 'uuid4', 'growl', 'WorkflowService', 'JointPaper', workflowsController
  ]);

function workflowsController($scope, $rootScope, $window, $state, $stateParams, $interval, $timeout, $uibModal, $log, $http, uuid4, growl, WorkflowService, JointPaper) {
  $scope.allowFlowIndicators = true
  $scope.JointPaper = JointPaper;
  $scope.criticResults = [];
  $scope.fedhubAlertsText = ""
  ...
  WorkflowService.isAllowFlowIndicators().then(function(allowFlowIndicators) {
      $scope.allowFlowIndicators = allowFlowIndicators
   })
  ...
  $interval(pollFedhubMetrics, 5000)

  function pollFedhubMetrics() {
    WorkflowService.getFedhubMetrics().then(function(metrics) {
      if ($scope.allowFlowIndicators) {
        var timestamp = new Date().getTime();
        translateIds();
        animateDataFlows(metrics.channelInfos, timestamp)
      }

      let totalMessagesDropped = metrics.totalMessagesDropped
      if (totalMessagesDropped && totalMessagesDropped > 0) {
        $scope.fedhubAlertsText = "Warning! " + totalMessagesDropped + " outgoing messages have been dropped by the server."
          + " If this number continues to grow at an unreasonable rate, consider upgrading your server to have more resources."
      } else {
        $scope.fedhubAlertsText = ""
      }
...
$scope.saveGraph = function() {
  ...
  var setupJointJsGraph = function() {
    ...
    function showPopover(targetElement, name) {
    ...
    JointPaper.paper.on('cell:mouseover', function(cellView, evt) {
      ...
      showPopover(cellView.$el, roger_federation.name, roger_federation.datasetName);
    });
