...
function manageController($scope, $rootScope, $state, $stateParams, $log, growl, ACCESS_EVENTS, RoleProductSetService,
  ...
  $scope.removeWorkflowSetRow = function(row) {
    bootbox.confirm("Are you sure you want to delete the workflow set (" + row.name + ")?", function(confirm) {
      if (confirm) {
        WorkflowSetsService.deleteWorkflowSet(row.id).then(function() {
          removeRowFromCollection(row);
        }, function(result) {
          var msg = "Failed to delete workflow sets file.";
          if (result.hasOwnProperty('data') && result.data != null && result.data.hasOwnProperty('error')) {
            msg = msg + " Error: " + result.data.error;
          }
          growl.error(msg);
        });
      }
  ...
  $scope.removeRoleProductSetRow = function(row) {
    WorkflowService.diagramsUsing(row.id).then(function(results) { //Check if the role/product set is in use first
      if (results.length === 0) {
        bootbox.confirm("Are you sure you want to delete the role/product set (" + row.name + ")?", function(confirm) {
          if (confirm) {
            RoleProductSetService.deleteRoleProductSet(row.id).then(function() {
              removeRowFromCollection(row);
            }, function(result) {
              var msg = "Failed to delete role-product set.";
              if (result.hasOwnProperty('data') && result.data != null && result.data.hasOwnProperty('message')) {
        ...
        var diagramNames = results.map(function(elem) {
          return elem.name;
        }).join(", ");
        growl.error("Role/Product Set is currently in use in the following diagrams (" + diagramNames + ")");
      }
    }, function(result) {
  ...
  $scope.removeWorkflowRow = function(row) {
    WorkflowService.diagramsUsing(row.id).then(function(results) { //Check if the Diagram is in use first
      if (results.length === 0) {
        bootbox.confirm("Are you sure you want to delete the " + row.diagramType + " (" + row.name + ")?", function(confirm) {
          if (confirm) {
            WorkflowService.deleteBpmnGraph(row.id).then(function() {
              removeRowFromCollection(row);
            }, function(result) {
              var msg = "Failed to delete " + row.diagramType;
              if (result.hasOwnProperty('data') && result.data != null && result.data.hasOwnProperty('error')) {
                msg = msg + " Error: " + result.data.error;
              }
              growl.error(msg);
        ...
        growl.error(row.diagramType + " is currently in use in the following diagrams (" + diagramNames + ")");
      }
    }, function(result) {
      growl.error(result.data.error);
    });
  ...
  $scope.removeInstanceRow = function(row) {
    bootbox.confirm("Are you sure you want to delete the instance (" + row.name + ")?", function(confirm) {
      if (confirm) {
        InstanceService.deleteInstance(row.id).then(function() {
          removeRowFromCollection(row);
        }, function(result) {
          var msg = "Failed to delete instance.";
          if (result.hasOwnProperty('data') && result.data != null && result.data.hasOwnProperty('error')) {
            msg = msg + " Error: " + result.data.error;
          }
          growl.error(msg);
  ...
  $scope.removeAssetRow = function(row) {
    bootbox.confirm("Are you sure you want to delete the role/product set (" + row.name + ")?", function(confirm) {
      if (confirm) {
        AssetService.deleteAssetFile(row.id).then(function() {
          removeRowFromCollection(row);
        }, function(result) {
          var msg = "Failed to delete asset file.";
          if (result.hasOwnProperty('data') && result.data != null && result.data.hasOwnProperty('error')) {
            msg = msg + " Error: " + result.data.error;
          }
          growl.error(msg);
