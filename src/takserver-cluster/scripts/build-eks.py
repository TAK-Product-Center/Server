...
KUBERNETES_SERVER_VERSION = '1.32'
...
TAK_CERT_SOURCE_DIR = os.environ['TAK_CERT_SOURCE_DIR']
...
# A global object to reflect the pod counts in use
class PodCounts:
    api = TAK_CLUSTER_NODE_COUNT
    messaging = max(1, int(round(TAK_CLUSTER_NODE_COUNT * TAK_MESSAGING_NODE_MULTIPLIER)))
    config = max(1, int(round(TAK_CLUSTER_NODE_COUNT * TAK_CONFIG_NODE_MULTIPLIER)))
    plugin = TAK_PLUGINS
    loadbalancer = max(1, int(round(TAK_CLUSTER_NODE_COUNT * TAK_LOAD_BALANCER_NODE_MULTIPLIER)))
    ignite = max(1, int(round(TAK_CLUSTER_NODE_COUNT * TAK_IGNITE_NODE_MULTIPLIER)))
    nats = max(1, int(round(TAK_CLUSTER_NODE_COUNT * TAK_NATS_NODE_MULTIPLIER)))
...
def setupECR():
    global AWS_ECR_URI
    try:
        find_ecr_res = boto3.client('ecr', region_name=TAK_CLUSTER_REGION).describe_repositories(
            repositoryNames=['tak/server-' + TAK_CLUSTER_NAME])
    ...
    while True:
        try:
            boto3.client('ecr', region_name=TAK_CLUSTER_REGION).describe_repositories(
                repositoryNames=['tak/server-' + TAK_CLUSTER_NAME])
    ...
    for _yaml in eks_yaml:
        ...
        for entry in _yaml['managedNodeGroups'][0]:
            if entry == 'desiredCapacity':
                _yaml['managedNodeGroups'][0][entry] = PodCounts.api
...
def adjustDeploymentsByNumberOfNodes():
    ...
    for _yaml in loadbalancer_yaml:
        if _yaml['kind'] == 'Deployment':
            _yaml['spec']['replicas'] = PodCounts.loadbalancer
...
def setupDBParameterGroups():
    try:
        find_db_param_group_res = boto3.client('rds', region_name=TAK_CLUSTER_REGION).describe_db_parameter_groups(
...
def setupSubnetGroups():
    try:
        find_db_subnet_group_res = boto3.client('rds', region_name=TAK_CLUSTER_REGION).describe_db_subnet_groups(
        ...
        try:
            find_db_subnets_res = boto3.client('ec2', region_name=TAK_CLUSTER_REGION).describe_subnets(Filters=[
                {'Name': 'tag:aws:cloudformation:stack-name', 'Values': ['eksctl-' + TAK_CLUSTER_NAME + '-cluster']}])

            subnets = []
            for subnet in find_db_subnets_res['Subnets']:
                subnets.append(subnet['SubnetId'])

            create_db_subnet_group_res = boto3.client('rds', region_name=TAK_CLUSTER_REGION).create_db_subnet_group(
                DBSubnetGroupName=TAK_CLUSTER_NAME + '-SG', DBSubnetGroupDescription=TAK_CLUSTER_NAME + '-SG',
                SubnetIds=subnets)
...
def describeSecurityGroups():
    try:
        find_security_groups_res = boto3.client('ec2', region_name=TAK_CLUSTER_REGION).describe_security_groups(
            Filters=[
                {'Name': 'tag:aws:cloudformation:stack-name', 'Values': ['eksctl-' + TAK_CLUSTER_NAME + '-cluster']}])
...
def setupRDS():
    ...
    while not rds_dns:
        ...
        try:
            find_db_instances_res = boto3.client('rds', region_name=TAK_CLUSTER_REGION).describe_db_instances(
...
def checkDBSetupDeploymentStatus():
    deployment_succeeded_code = runCmd(
        "kubectl get pod --field-selector=status.phase=Succeeded | grep takserver-database-setup")
    deployment_failed_code = runCmd(
        "kubectl get pod --field-selector=status.phase=Failed | grep takserver-database-setup")
...
def deployDatabaseSetupPod():
    ...
    while True:
        db_deployment_failed_code = runCmd(
            "kubectl get pod --field-selector=status.phase=Failed | grep takserver-database-setup")
        db_deployment_succeeded_code = runCmd(
            "kubectl get pod --field-selector=status.phase=Succeeded | grep takserver-database-setup")
...
def validateInputData():
    if TAK_CERT_SOURCE_DIR != '':
        if not os.path.isdir(TAK_CERT_SOURCE_DIR):
            print(f"The specified value for TAK_CERT_SOURCE_DIR {TAK_CERT_SOURCE_DIR} does not exist!", file=sys.stderr)
            exit(1)

        for cert in REQUIRED_CERTIFICATES:
            if not os.path.isfile(os.path.join(TAK_CERT_SOURCE_DIR, cert)):
                print(f'Although the certificate directory {TAK_CERT_SOURCE_DIR} exists it does not contain '
                                f'the required certificate {cert}!', file=sys.stderr)
                exit(1)

# Generate Cluster Certificaties If They Dont Exist
def generateTakseverCertificates():
    if TAK_CERT_SOURCE_DIR != '':
        shutil.copytree(TAK_CERT_SOURCE_DIR, CLUSTER_HOME_DIR + '/takserver-core/certs/files')

    if not os.path.isdir(CLUSTER_HOME_DIR + '/takserver-core/certs/files') or not len(os.listdir(
            CLUSTER_HOME_DIR + '/takserver-core/certs/files')) > 0:
        if not TAK_CA_NAME or not TAK_CITY or not TAK_STATE or not TAK_ORGANIZATIONAL_UNIT:
            printJson(
                'No certs found and cert metadata not defined in cluster-properties. please update cluster-properties, resource the file, and try rebuilding')
            sys.exit(1)

        runCmd(
            'cd ' + CLUSTER_HOME_DIR + ' && docker build -t ca-setup -f docker-files/Dockerfile.ca . && docker run -it --name ca-setup-container --env CA_NAME=' + TAK_CA_NAME + ' --env STATE=' + TAK_STATE + ' --env CITY=' + TAK_CITY + ' --env ORGANIZATIONAL_UNIT=' + TAK_ORGANIZATIONAL_UNIT + ' ca-setup > /dev/null && docker cp ca-setup-container:/files ' + CLUSTER_HOME_DIR + '/takserver-core/certs/ && docker rm ca-setup-container')
        print(
...
def addIgniteConfigMap():
    ...
    config_map_cmd = 'kubectl create configmap ignite-extensions --from-file=' + CLUSTER_HOME_DIR + '/takserver-core/ignite-extensions.jar --dry-run=client -o yaml >' + CLUSTER_HOME_DIR + '/deployments/helm/templates/ignite-extensions.yaml'
    runCmd(config_map_cmd)
...
def publishTakserverCoreImages():
    base_cmd = (
                'cd ' + CLUSTER_HOME_DIR + ' && docker build -t ' + AWS_ECR_URI + ':core-base -f docker-files/Dockerfile.takserver-base . && docker push ' + AWS_ECR_URI + ':core-base')

    cmd_status = runCmd(base_cmd)

    if cmd_status == 0:
        build_dependent_dockerfiles_cmd = (
                    'cd ' + CLUSTER_HOME_DIR + ' && docker build -t ' + AWS_ECR_URI + ':messaging-provisioned -f docker-files/Dockerfile.takserver-messaging --build-arg TAKSERVER_IMAGE_REPO=' + AWS_ECR_URI + ' .'
...
# Deploy Takserver Ignite
def publishTakserverIgniteImage():
    cmd = 'cd ' + CLUSTER_HOME_DIR + ' && docker build -t ' + AWS_ECR_URI + ':ignite-provisioned -f docker-files/Dockerfile.takserver-ignite --build-arg TAKSERVER_IMAGE_REPO=' + AWS_ECR_URI + ' . && docker push ' + AWS_ECR_URI + ' --all-tags'
    cmd_status = runCmd(cmd)

    if cmd_status == 0:
        print("\nTakserver Ignite Image deployed")
    else:
        print("\nTakserver Ignite Image was not deployed")
...
def getLoadBalancerDNS():
    ...
    while not load_balancer_active:
        ...
        for load_balancer in load_balancers['LoadBalancers']:
            try:
                cluster_load_balancer = boto3.client('elbv2', region_name=TAK_CLUSTER_REGION).describe_tags(
...
def createCNAMEForLoadBalancerDNS(zone, dns):
    ...
    try:
        create_resource_record_sets_res = boto3.client('route53',
                                                       region_name=TAK_CLUSTER_REGION).change_resource_record_sets(
            HostedZoneId=zone,
            ChangeBatch={
                'Changes': [
                    {
    ...
    try:
        ...
        for network in core_config.getroot().findall('{http://bbn.com/marti/xml/config}network'):
            for input in network.findall('{http://bbn.com/marti/xml/config}input'):
                print('\nClients can now connect to Taksever at ' + domain_name + ':' + input.get(
                    'port') + ' using protocol '
...
def installHelmChart(dry_run=False):
    ...
    runCmd(('helm upgrade' + (' --dry-run' if dry_run else '')
            ...
            + ' --set takserver.plugins.enabled=' + str(PodCounts.plugin)
            + ' --set takserver.config.replicas=' + str(PodCounts.config)
            + ' --set takserver.messaging.replicas=' + str(PodCounts.messaging)
            + ' --set takserver.api.replicas=' + str(PodCounts.api)
            ...
            + ' --set ignite.replicaCount=' + str(PodCounts.ignite)
            + ' --set nats.cluster.replicas=' + str(PodCounts.nats)
            + ' --set stan.cluster.replicas=' + str(PodCounts.nats)
            + ' --set global.postgresql.host=' + rds_dns
