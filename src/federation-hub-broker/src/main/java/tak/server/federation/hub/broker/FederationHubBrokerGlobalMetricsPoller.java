...
public class FederationHubBrokerGlobalMetricsPoller implements DisposableBean {

    private static final Logger logger = LoggerFactory.getLogger(FederationHubBrokerGlobalMetricsPoller.class);

    private final ScheduledFuture<?> scheduleFuture;

    public FederationHubBrokerGlobalMetricsPoller() {
        logger.info("Initializing federation hub broker global metrics poller.");
