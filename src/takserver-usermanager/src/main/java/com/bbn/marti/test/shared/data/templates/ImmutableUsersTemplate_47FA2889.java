...
public class ImmutableUsersTemplate_47FA2889 extends AbstractUser {
	...
	public final Path getCertPublicPemPath() {
		if (certPublicPemPath == null) return null;
		
		if (SSLHelper.useUniqueCertsPerServer()) {
			String serverId = this.getServer().getConsistentUniqueReadableIdentifier();
			String publicCertFile = TAKCLConfigModule.getInstance().getTemporaryDirectory() + "/" + serverId + "/" + username + ".pem";
			return Paths.get(publicCertFile);
		} else {
	...
	public final Path getCertPrivateJksPath() {
		if (certPrivateJksPath == null) return null;

		if (SSLHelper.useUniqueCertsPerServer()) {
			String serverId = this.getServer().getConsistentUniqueReadableIdentifier();
			String privateCertFile = TAKCLConfigModule.getInstance().getTemporaryDirectory() + "/" + serverId + "/" + username + ".jks";
			return Paths.get(privateCertFile);
		} else {
