...
public class MissionApi extends BaseRestController {
	...
	Callable<ApiResponse<Set<Mission>>> getMissionByGuid(
		...
		return () -> {
			...
			try {
				...
				logger.debug("mission from getMissionByGuid {} {}", missionGuid, mission);
				...
				logger.debug("GET mission: {}", mission);
	...
	private ApiResponse<Set<Mission>> doCreateMissionAllowDupe(String nameParam,
															   ...
															   boolean allowDupe,
															   boolean allowDupeInDifferentGroups,
															   byte[] requestBody,
		...
		try {
    		...
    		if (bitVectorUser.and(bitVectorMission).compareTo(bitVectorMission) != 0) {
    			...
    			if (groupNames.length == 1 && groupNames[0].equals("__ANON__")) {
					...
					bitVectorMission = bitVectorUser;
				} else {
			...
			try {
				...
				// get mission
				try {
					mission = missionService.getMissionByName(name, true);
					missionService.validateMission(mission, name); // checks if the mission was deleted, or null
				} catch (Exception e) {
					if (logger.isDebugEnabled()) {
				...
				// If the mission exists, but the user is not allowed to access it, don't use or try to update the existing mission. Instead, allow duplicate mission creation. 
				if (!remoteUtil.isGroupVectorAllowed(groupVectorUser, mission.getGroupVector())) {
					allowDupe = true;
					mission = null;
				}

				if (allowDupe && allowDupeInDifferentGroups) {
	...
	private Mission doInternalCreateMission(
		...
		logger.info("Creating new mission {}", name);
	...
	ApiResponse<Set<Mission>> deleteMissionByGuid(
		...
		try {
			...
			guid = UUID.fromString(guidString);
		} catch (IllegalArgumentException e) {
	...
	/*
	 * Send a mission package to a list of contacts (by mission name)
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	...
	/*
	 * Send a mission package to a list of contacts (by mission guid)
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{guid:.+}/send", method = RequestMethod.POST)
	ApiResponse<Set<Mission>> sendMissionArchiveByGuid(@PathVariable("name") @NotNull String missionGuidParam, HttpServletRequest request) throws ValidationException, IntrusionException {
	...
	/*
	 * add single or multiple mission content (hash or UID) - referencing mission by name.
	 *
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	...
	/*
	 * add single or multiple mission content (hash or UID) - referencing mission by guid.
	 *
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	...
	@RequestMapping(value = "/missions/guid/{guid:.+}/contents", method = RequestMethod.DELETE)
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	ApiResponse<Set<Mission>> removeMissionContentByGuid(@PathVariable("guid") String guid,
												   @RequestParam(value = "hash", required = false) @ValidatedBy("MartiSafeString") String hash,
												   @RequestParam(value = "uid", required = false) @ValidatedBy("MartiSafeString") String uid,
												   @RequestParam(value = "creatorUid", defaultValue = "") @ValidatedBy("MartiSafeString") String creatorUid,
												   HttpServletRequest request) throws ValidationException, IntrusionException {
		...
		// validate this differently since it's a path variable
		validator.getValidInput(context, guid, "MartiSafeString", DEFAULT_PARAMETER_LENGTH, false);
	...
	/*
	 * Returns the mission subscription for the current user by guid
	 */
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscription", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ApiResponse<MissionSubscription> getSubscriptionForUserByGuid(
			...
			@RequestParam(value = "uid", defaultValue = "") String uid)
	{
		...
		final String username = SecurityContextHolder.getContext().getAuthentication().getName();
		MissionSubscription missionSubscription = missionSubscriptionRepository
				.findByMissionGuidAndClientUidAndUsernameNoMission(missionGuid.toString(), uid, username);
	...
	/*
	 * subscribe to mission changes by guid
	 */
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscription", method = RequestMethod.PUT)
	@ResponseStatus(HttpStatus.CREATED)
	public Callable<ApiResponse<MissionSubscription>> createMissionSubscriptionByGuid(
		...
		final UUID missionGuid = parseGuid(missionGuidParam);
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_SET_ROLE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscription", method = RequestMethod.POST)
	public void setSubscriptionRoleByGuid(
			@RequestBody List<MissionSubscription> subscriptions,
			@PathVariable("missionGuid") String missionGuidParam,
			@RequestParam(value = "creatorUid", required = true) @ValidatedBy("MartiSafeString") String creatorUid,
			HttpServletRequest request) {
	...
	/*
	 * unsubscribe from mission changes
	 *
	 */
	...
	/*
	 * unsubscribe from mission changes by guid
	 *
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')") // use MISSION_READ here to allow readonly users to unsubscribe
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscription", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.OK)
	public Callable<Void> deleteMissionSubscriptionByGuid(
			@RequestParam(value = "uid", defaultValue = "") String uid,
			@RequestParam(value = "topic", defaultValue = "") String topic,
			@RequestParam(value = "disconnectOnly", defaultValue = "true") boolean disconnectOnly,
			@PathVariable("missionGuid") String missionGuidParam) {
	...
	/*
	 * Get all mission subscriptions with guid
	 *
	 */
	@PreAuthorize("hasRole('ROLE_ADMIN')") // restrict to admin
	@RequestMapping(value = "/missions/all/subscriptions/guid", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ApiResponse<List<Map.Entry<Map.Entry<String, String>, String>>> getAllMissionSubscriptionsWithGuid() {
		return new ApiResponse<List<Map.Entry<Map.Entry<String, String>, String>>>(Constants.API_VERSION, "MissionSubscription", missionService.getAllMissionSubscriptionsWithGuid());
	}
	...
	/*
	 * 
	 * Get subscriptions to the specified mission by guid
	 *
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscriptions", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ApiResponse<List<String>> getMissionSubscriptionsByGuid(
			@PathVariable("missionGuid") String missionGuidParam, HttpServletRequest request) {
	...
	/* roles by guid */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/subscriptions/roles", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ApiResponse<List<MissionSubscription>> getMissionSubscriptionRolesByGuid(
			@PathVariable("missionGuid") String missionGuidParam, HttpServletRequest request) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/role", method = RequestMethod.GET)
	public ApiResponse<MissionRole> getMissionRoleFromTokenByGuid(
			@PathVariable("missionGuid") String missionGuidParam, HttpServletRequest request) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_SET_ROLE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/role", method = RequestMethod.PUT)
	public void setMissionRoleByGuid(
			@PathVariable("missionGuid") String missionGuidParam,
	...
	/*
	 * Get invitations to the specified mission by guid
	 *
	 */
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/invitations", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ApiResponse<List<MissionInvitation>> getMissionInvitationsByGuid(
			@PathVariable("missionGuid") String missionGuidParam, HttpServletRequest request) {
	...
	@ResponseStatus(HttpStatus.OK)
	@Transactional
	public void uninviteFromMissionByGuid(
			@PathVariable("missionGuid") @NotNull String missionGuidParam,
			@PathVariable("type") @NotNull MissionInvitation.Type type,
			@PathVariable("invitee") @NotNull String invitee,
			@RequestParam(value = "creatorUid", required = true) @ValidatedBy("MartiSafeString") String creatorUid,
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/contacts", method = RequestMethod.GET)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<List<RemoteSubscription>> getMissionContactsByGuid(
			@PathVariable("missionGuid") @NotNull String missionGuidParam, HttpServletRequest request) throws RemoteException {
	...
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<List<RemoteSubscription>> getMissionContacts(
	...
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/invite", method = RequestMethod.POST)
	public void sendMissionInvitesByGuid(
			@PathVariable("missionGuid") @NotNull String missionGuidParam,
			@RequestParam(value = "creatorUid", required = false) @ValidatedBy("MartiSafeString") String creatorUid,
			HttpServletRequest request
	) throws RemoteException, ValidationException, IOException {
		...
		if (missionService.getApiVersionNumberFromRequest(request) > 2) {
			...
			for (String uid : contactUids) {
				try {
					...
					logger.debug("Attempt to re-invite clientUid: " + uid + " to: " + mission.getName() + " " + mission.getGuid());
					continue;
				}
			}
		}
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{childGuid:.+}/parent/guid/{parentGuid:.+}", method = RequestMethod.PUT)
	@ResponseStatus(HttpStatus.OK)
	public void setParentByGuid(@PathVariable("childGuid") @NotNull String childGuidParam,
			@PathVariable("parentGuid") @NotNull String parentGuidParam,
			HttpServletRequest request) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{childGuid:.+}/parent", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.OK)
	public void clearParentByGuid(@PathVariable("childGuid") @NotNull String childGuidParam, HttpServletRequest request) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/kml", method = RequestMethod.GET)
	ResponseEntity<String> getKmlByGuid(
			@PathVariable("missionGuid") @NotNull String missionGuidParam,
			@RequestParam(value = "download", defaultValue = "false") boolean download,
			HttpServletRequest request) {
		String groupVector = martiUtil.getGroupVectorBitString(request);
		...
		String urlBase = null;
		if (download) {
			headers.setContentDispositionFormData("kml", missionGuid + ".kml");
		} else {
			String requestUrl = request.getRequestURL().toString();
			try {
				requestUrl = URLDecoder.decode(requestUrl, "UTF-8");
		...
		return new ResponseEntity<String>(
				missionService.getMissionKml(missionGuid, urlBase, groupVector), headers, HttpStatus.OK);
	}
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{guid}/externaldata", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.CREATED)
	public ApiResponse<ExternalMissionData> setExternalMissionDataByGuid(
			@PathVariable(value = "guid") String missionGuidParam,
			@RequestParam(value = "creatorUid") @ValidatedBy("MartiSafeString") String creatorUid,
			@RequestBody @NotNull ExternalMissionData externalMissionData,
			HttpServletRequest request) {
		if (Strings.isNullOrEmpty(externalMissionData.getId())) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{guid}/externaldata/{id}", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.OK)
	public void deleteExternalMissionDataByGuid(
			@PathVariable(value = "guid") String missionGuidParam,
			@PathVariable(value = "id") String externalMissionDataId,
			@RequestParam(value = "notes") @ValidatedBy("MartiSafeString") String notes,
			@RequestParam(value = "creatorUid") @ValidatedBy("MartiSafeString") String creatorUid,
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{guid}/externaldata/{id}/change", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	public void notifyExternalDataChangedByGuid(
			@PathVariable(value = "guid") String missionGuidParam,
			@PathVariable(value = "id") String externalMissionDataId,
			@RequestParam(value = "creatorUid") @ValidatedBy("MartiSafeString") String creatorUid,
			@RequestParam(value = "notes") @ValidatedBy("MartiSafeString") String notes,
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_SET_PASSWORD')")
	@RequestMapping(value = "/missions/guid/{guid:.+}/password", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.OK)
	public void removePasswordByGuid(
			@PathVariable(value = "guid") @ValidatedBy("MartiSafeString") String missionGuidParam,
			@RequestParam(value = "creatorUid", defaultValue = "") @ValidatedBy("MartiSafeString") String creatorUid,
			HttpServletRequest request) {
	...
	public void setExpirationByGuid(
		...
		try {
			...
			try {
				if (logger.isDebugEnabled()) {
					logger.debug(" setting the mission expiration " + missionGuid.toString() + " time " + expiration);
				}
				retentionPolicyConfig.setMissionExpiryTask(missionGuid.toString(), expiration);
			} catch (Exception e) {
				logger.error(" Exception getting Retention service, task not scheduled immediately " + missionGuid.toString());
			}
		} catch (Exception e) {
			logger.debug("exception setting mission expiration " + e.getMessage(), e);
		}
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/maplayers", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	ApiResponse<MapLayer> createMapLayerByGuid(
		...
		MapLayer newMapLayer = missionService.addMapLayerToMission(mission.getName(), creatorUid, mission, mapLayer);
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/{missionGuid:.+}/maplayers/{uid}", method = RequestMethod.DELETE)
	@ResponseStatus(HttpStatus.OK)
	void deleteMapLayerByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@RequestParam(value = "creatorUid") @ValidatedBy("MartiSafeString") String creatorUid,
			@PathVariable("uid") @NotNull String uid) {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/maplayers", method = RequestMethod.PUT)
	@ResponseStatus(HttpStatus.OK)
	ApiResponse<MapLayer> updateMapLayerByGuid(
		...
		MapLayer newMapLayer = missionService.updateMapLayer(mission.getName(), creatorUid, mission, mapLayer);
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers", method = RequestMethod.GET)
	public ApiResponse<List<MissionLayer>> getMissionLayersByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam)
					throws ValidationException, IntrusionException, RemoteException {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_READ')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers/{layerUid:.+}", method = RequestMethod.GET)
	public ApiResponse<MissionLayer> getMissionLayerByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@PathVariable(value = "layerUid") String layerUid)
			throws ValidationException, IntrusionException, RemoteException {
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers", method = RequestMethod.PUT)
	public ApiResponse<MissionLayer> createMissionLayerByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@RequestParam(value = "name", required = true) @ValidatedBy("MartiSafeString") String name,
			@RequestParam(value = "type", required = true) @ValidatedBy("MartiSafeString") MissionLayer.Type type,
			@RequestParam(value = "uid", required = false) @ValidatedBy("MartiSafeString") String uid,
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers/{layerUid:.+}/name", method = RequestMethod.PUT)
	public void setLayerNameByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@PathVariable(value = "layerUid") String layerUid,
			@RequestParam(value = "name", required = true) @ValidatedBy("MartiSafeString") String name,
			@RequestParam(value = "creatorUid", required = true) @ValidatedBy("MartiSafeString") String creatorUid)
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionName:.+}/layers/{layerUid:.+}/position", method = RequestMethod.PUT)
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers/{layerUid:.+}/position", method = RequestMethod.PUT)
	public void setLayerPositionByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@PathVariable(value = "layerUid") String layerUid,
			@RequestParam(value = "afterUid", required = false) @ValidatedBy("MartiSafeString") String afterUid,
			@RequestParam(value = "creatorUid", required = true) @ValidatedBy("MartiSafeString") String creatorUid)
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers/parent", method = RequestMethod.PUT)
	public void setLayerParentByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@RequestParam(value = "layerUid") @ValidatedBy("MartiSafeString") String[] layerUids,
			@RequestParam(value = "parentUid", required = false) @ValidatedBy("MartiSafeString") String parentUid,
			@RequestParam(value = "afterUid", required = false) @ValidatedBy("MartiSafeString") String afterUid,
		...
		String useAfterUid = afterUid;
		for (String layerUid : layerUids) {
			missionService.setLayerParent(mission.getName(), mission, layerUid, parentUid, useAfterUid, creatorUid);
			// move each other layer in the last after its predecessor in the parameter list
			useAfterUid = layerUid;
		}
	}
	...
	@PreAuthorize("hasPermission(#request, 'MISSION_WRITE')")
	@RequestMapping(value = "/missions/guid/{missionGuid:.+}/layers", method = RequestMethod.DELETE)
	public void deleteMissionLayerByGuid(
			@PathVariable(value = "missionGuid") String missionGuidParam,
			@RequestParam(value = "uid", required = true) @ValidatedBy("MartiSafeString") String[] layerUids,
			@RequestParam(value = "creatorUid", required = true) @ValidatedBy("MartiSafeString") String creatorUid)
					throws ValidationException, IntrusionException, RemoteException {
	...
	/*
	 * get all missions with pagination (used by mission manager UI)
	 */
