...
public interface MissionChangeRepository extends JpaRepository<MissionChange, Long> {
    ...
    static final String MISSION_CREATES_AND_DELETES =
            "select id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, 0 as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes\n" +
    ...
    static final String HASH_ADDS =
            "select max(mc.id) as id, 2 as change_type, mc.hash as hash, max(mc.ts) as ts, max(mc.servertime) as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, 0 as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes\n" +
                    "from mission_change mc \n" +
    ...
    static final String HASH_ADDS_FULL_HISTORY =
            "select mc.id as id, 2 as change_type, mc.hash as hash, mc.ts as ts, mc.servertime as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, 0 as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes\n" +
                    "from mission_change mc \n" +
                    "inner join resource r on mc.hash = r.hash\n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
    ...
    static final String UID_ADDS =
            "select max(mc.id) as id, 2 as change_type, null as hash, max(mc.ts) as ts, max(mc.servertime) as servertime, mc.uid as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, 0 as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes\n" +
                    "from mission_change mc\n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
    ...
    static final String MISSION_CHANGES = MISSION_CREATES_AND_DELETES + "union all " + HASH_ADDS + "union all " +  HASH_REMOVES + "union all " + UID_ADDS + "union all " + UID_REMOVES + "union all " + EXTERNAL_DATA_ADDS + "union all " + EXTERNAL_DATA_REMOVES;
    static final String MISSION_CHANGES_FULL_HISTORY = MISSION_CREATES_AND_DELETES + "union all " + HASH_ADDS_FULL_HISTORY + "union all " +  HASH_REMOVES_FULL_HISTORY + "union all " + UID_ADDS_FULL_HISTORY + "union all " + UID_REMOVES_FULL_HISTORY + "union all " + EXTERNAL_DATA_ADDS_FULL_HISTORY + "union all " + EXTERNAL_DATA_REMOVES_FULL_HISTORY;
...
//    @Cacheable(cacheResolver = MissionChangeCacheResolver.MISSION_CHANGE_CACHE_RESOLVER)
    @Query(value = "select distinct on (uid) id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, 0 as mission_createTime, " +
            "null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes " +
