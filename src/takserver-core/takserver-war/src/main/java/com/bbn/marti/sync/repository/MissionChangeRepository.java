...
public interface MissionChangeRepository extends JpaRepository<MissionChange, Long> {
    ...
    static final String MISSION_CREATES_AND_DELETES =
            "select id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, to_timestamp(0) as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid\n" +
                    "from mission_change mc\n" +
                    "where lower(mission_name) = lower(:missionName)\n" +
                    "and change_type between 0 and 1\n" + // mission create and delete change types
                    "and ts >= (select max(ts) from mission_change where lower(mission_name) = lower(:missionName) and change_type = 0)\n" +
                    START_END_PRED;

    static final String HASH_ADDS =
            "select max(mc.id) as id, 2 as change_type, mc.hash as hash, max(mc.ts) as ts, max(mc.servertime) as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid \n" +
                    "from mission_change mc \n" +
                    ...
                    "and lower(m.name) = lower(:missionName)\n" +
                    "and mc.ts >= m.create_time\n" +
                    START_END_PRED +
                    "group by mc.hash, mc.creatoruid\n";

    static final String HASH_ADDS_FULL_HISTORY =
            "select mc.id as id, 2 as change_type, mc.hash as hash, mc.ts as ts, mc.servertime as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid \n" +
                    "from mission_change mc \n" +
                    "inner join resource r on mc.hash = r.hash\n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
    ...
    static final String UID_ADDS =
            "select max(mc.id) as id, 2 as change_type, null as hash, max(mc.ts) as ts, max(mc.servertime) as servertime, mc.uid as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid \n" +
                    "from mission_change mc\n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
    ...
    static final String EXTERNAL_DATA_ADDS =
            "select max(mc.id) as id, 2 as change_type, null as hash, max(mc.ts) as ts, max(mc.servertime) as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, mc.external_data_uid as external_data_uid, mc.external_data_name as external_data_name, mc.external_data_tool as external_data_tool, mc.external_data_token as external_data_token, mc.external_data_notes as external_data_notes, null as mission_feed_uid, null as map_layer_uid  \n" +
                    "from mission_change mc \n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
    ...
    static final String EXTERNAL_DATA_ADDS_FULL_HISTORY =
            "select mc.id as id, 2 as change_type, null as hash, mc.ts as ts, mc.servertime as servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, mc.external_data_uid as external_data_uid, mc.external_data_name as external_data_name, mc.external_data_tool as external_data_tool, mc.external_data_token as external_data_token, mc.external_data_notes as external_data_notes, null as mission_feed_uid, null as map_layer_uid \n" +
                    "from mission_change mc \n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
                    "left join mission_external_data med on med.id = mc.external_data_uid and med.mission_id = m.id\n" + //this join needs two criteria in the case of multiple missions and the same resource / or same hash different resource row
                    "where \n" +
    ...
    static final String MAP_LAYERS =
            "select mc.id as id, change_type, null as hash, ts, servertime, null as uid, mc.creatoruid as creatoruid, null as mission_id, :missionName as mission_name, to_timestamp(0) as mission_createTime, external_data_uid, external_data_name, external_data_tool, external_data_token, external_data_notes, mission_feed_uid, map_layer_uid \n" +
                    "from mission_change mc \n" +
                    "inner join mission m on mc.mission_id = m.id\n" +
                    "where \n" +
...
//    @Cacheable(cacheResolver = MissionChangeCacheResolver.MISSION_CHANGE_CACHE_RESOLVER)
    @Query(value = "select distinct on (uid) id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, to_timestamp(0) as mission_createTime, " +
            ...
            "    from mission_change mc where lower(mission_name) = lower(:missionName) and change_type = :changeType and " +
            "    uid IN (:uids) " +
...
//    @Cacheable(cacheResolver = MissionChangeCacheResolver.MISSION_CHANGE_CACHE_RESOLVER)
    @Query(value = "select distinct on (hash) id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, to_timestamp(0) as mission_createTime, " +
            "null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid  " +
            "    from mission_change mc where lower(mission_name) = lower(:missionName) and change_type = :changeType and " +
            "    hash IN (:hashes) " +
...
//    @Cacheable(cacheResolver = MissionChangeCacheResolver.MISSION_CHANGE_CACHE_RESOLVER, key="{#root.methodName, #root.args[0]}")
    @Query(value = "select distinct on (uid, hash) id, change_type, hash, ts, servertime, uid, mc.creatoruid as creatoruid, null as mission_id, mission_name, to_timestamp(0) as mission_createTime, " +
            "null as external_data_uid, null as external_data_name, null as external_data_tool, null as external_data_token, null as external_data_notes, null as mission_feed_uid, null as map_layer_uid  " +
            "    from mission_change mc where lower(mission_name) = lower(:missionName) and change_type = :changeType and " +
    ...
    @Query(value = "select change_type from mission_change where lower(mission_name) = lower(:missionName) order by id desc limit 1;", nativeQuery = true)
    MissionChangeType getLatestChangeTypeForMission(@Param("missionName") String missionName);
