...
public interface MissionSubscriptionRepository extends JpaRepository<MissionSubscription, String> {
    ...
    @Modifying
    @Transactional
    @Query(value = "insert into mission_subscription ( mission_id, client_uid, create_time, uid, token, role_id ) " +
            " values ( :missionId, :clientUid, :createTime, :uid, :token, :roleId ) ", nativeQuery = true)
    ...
    void subscribe(@Param("missionId") long missionId, @Param("clientUid") String clientUid, @Param("createTime") Date createTime,
                   @Param("uid") String uid, @Param("token") String token, @Param("roleId") long roleId);
    @Modifying
    @Transactional
    ...
    @Query(value = "select ms.uid, ms.token, null as mission_id, ms.client_uid, ms.create_time, ms.role_id from mission m " +
    ...
    @Query(value = "select ms.uid, null as token, null as mission_id, ms.client_uid, ms.create_time, ms.role_id from mission m " +
            "inner join mission_subscription ms on m.id = ms.mission_id where m.name = :missionName", nativeQuery = true)
    @Cacheable(Constants.MISSION_SUBSCRIPTION_CACHE)
    List<MissionSubscription> findAllByMissionNameNoMissionNoToken(@Param("missionName") String missionName);

    @Query(value = "select ms.uid, ms.token, m.id as mission_id, ms.client_uid, ms.create_time, ms.role_id from mission m " +
