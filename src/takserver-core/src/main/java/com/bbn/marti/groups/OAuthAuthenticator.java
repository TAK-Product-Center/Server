...
public class OAuthAuthenticator extends AbstractAuthenticator implements Serializable {
	...
	private String groupNameExtractorRegex;
	...
	public OAuthAuthenticator(GroupManager groupManager, ActiveGroupCacheHelper activeGroupCacheHelper) {
		...
		oAuth2AuthorizationService = SpringContextBeanForApi.getSpringContext()
				.getBean(OAuth2AuthorizationService.class);

		if (oauthConf != null) {
			groupPrefix = oauthConf.getGroupprefix().toLowerCase();
			groupNameExtractorRegex = oauthConf.getGroupNameExtractorRegex();
	...
	private AuthResult auth(User user) {
		...
		try {
			...
			if (oauthConf != null && oauthConf.getUsernameClaim() != null
				...
				// For other trusted tokens, assign a random username if we don't have an
				// attribute mapping
			...
			if (user instanceof AuthenticatedUser) {
				...
				user = new AuthenticatedUser(username, auser.getConnectionId(), auser.getAddress(), auser.getCert(),
						username, "", "", auser.getConnectionType()); // no password or uid
				user.setToken(token);
			}
			...
			if (country != null && classification != null) {
				...
				UserClassification userClassification = new UserClassification(country, classification, accms,
						sciControls);
			...
			// disable the group cache when retrieving the enrollment profile to allow clients to download
			// the profile prior to channel selection. this is needed when using token enrollment
			HttpServletRequest request = ThreadLocalRequestHolder.getRequest();
			if (request != null) {
				if (request.getRequestURI().equals("/Marti/api/tls/profile/enrollment")) {
					useGroupCache = false;
				}
			}

			// if the token contains an admin claim, add the admin role to the user
			if (hasAdminTokenClaim(claims)) {
				user.getAuthorities().add("ROLE_ADMIN");
			}

			if (groupNames != null) {
				Set<String> groupNameSet = LdapAuthenticator.applyGroupPrefixFilter(new HashSet<String>(groupNames),
						groupPrefix);

				if (!Strings.isNullOrEmpty(groupNameExtractorRegex)) {
					GroupNameExtractor groupNameExtractor = new GroupNameExtractor(groupNameExtractorRegex);
					Set<String> temp = new HashSet<>();
					groupNameSet.forEach(groupName -> {
						temp.add(groupNameExtractor.extractGroupName(groupName));
					});
					groupNameSet = temp;
				}

				Set<Group> groups = new ConcurrentSkipListSet<>();
				LdapAuthenticator.groupNamesToGroups(groupManager, groupNameSet, groups, readOnlyGroup, readGroupSuffix,
						writeGroupSuffix);
				...
				if (auth.compareToIgnoreCase("file") == 0) {
						...
						if (fileUser.getIdentifier().compareTo(username) == 0) {
							...
							if (useGroupCache) {
								...
								if (activeGroupCacheHelper.assignGroupsCheckCache(hydrated, user, username)) {
									// notify the user that their cache has been updated
									try {
										DistributedSubscriptionManager.getInstance().sendGroupsUpdatedMessage(username,
												null);
									} catch (Exception e) {
										if (logger.isDebugEnabled()) {
											logger.debug("exception calling sendGroupsUpdatedMessage!", e);
					...
					// if the LDAP authenticator is configured, use it to assign groups for the
					// user, using the service credentials. Can be disabled by setting the
					// x509groups option to false.
					try {
						if (ldapConf != null
								&& CoreConfigFacade.getInstance().getRemoteConfiguration().getAuth().isX509Groups()
								&& ldapConf.isX509Groups()) {
							...
							try {
								...
								if (!groupInfo.isEmpty()) {
									...
									if (useGroupCache) {
										...
										readOnly = LdapAuthenticator.getInstance().groupNamesToGroups(setGroupNames,
												ldapGroups);
										...
										if (activeGroupCacheHelper.assignGroupsCheckCache(ldapGroups, user, username)) {
											...
											try {
												DistributedSubscriptionManager.getInstance()
														.sendGroupsUpdatedMessage(username, null);
											} catch (Exception e) {
												if (logger.isDebugEnabled()) {
													logger.debug("exception calling sendGroupsUpdatedMessage!", e);
												}
								...
								if (ldapConf.isX509AddAnonymous() && CoreConfigFacade.getInstance()
										.getRemoteConfiguration().getAuth().isX509AddAnonymous()) {
	...
	private boolean hasAdminTokenClaim(Claims claims) {
		if (oauthConf != null) {
			String adminTokenClaimName = oauthConf.getAdminTokenClaimName();
			String adminTokenClaimValue = oauthConf.getAdminTokenClaimValue();

			if (!Strings.isNullOrEmpty(adminTokenClaimName) && !Strings.isNullOrEmpty(adminTokenClaimValue)) {
				Object adminClaim = claims.get(adminTokenClaimName);
				if (adminClaim instanceof ArrayList<?> list) {
					return list.contains(adminTokenClaimValue);
				} else if (adminClaim instanceof String s) {
					return s.equals(adminTokenClaimValue);
				}
			}
		}
