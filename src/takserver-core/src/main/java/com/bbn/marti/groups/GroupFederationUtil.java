...
public class GroupFederationUtil {
	...
	public void trackLatestSA(Subscription subscription, CotEventContainer cot, boolean force) {
		...
		if (cot.getUid() != null && subscription.clientUid != null
				&& (force || cot.getUid().equals(subscription.clientUid))) {
			...
			if (CoreConfigFacade.getInstance().getRemoteConfiguration().getCluster().isEnabled()
					&& subscription.getUser().getConnectionId() != null) {
				IgniteCacheHolder.getIgniteLatestSAConnectionUidCache().put(subscription.getUser().getConnectionId(),
						cot.asXml());
			}
	...
	public Set<Subscription> getReachableFederatedGroupMappingSubscriptons(Subscription sub) {
		// since federated group mapping doesnt follow traditional group reachability
		// flow,
		// we need to check each FigFederateSubscription individually to determine
		// reachability
		...
		for (FederateSubscription destSub : SubscriptionStore.getInstanceFederatedSubscriptionManager()
			...
			if (destSub instanceof FigFederateSubscription) {
				...
				boolean isNotFedToFed = srcFedUser == false
						|| (srcFedUser && !(destFedSub.getUser() instanceof FederateUser));
	...
	public void updateInputGroups(@NotNull Input input) {
		...
		// If a user exists for this connectionId, use it. Otherwise, create a new one
		// and put him into the group manager
		User user = null;

		user = groupManager.getUserByConnectionId(connectionId);

		if (user == null) {
			user = new AuthenticatedUser(connectionId, connectionId, "127.0.0.1", null, "dummyLogin", "dummyPassword",
					"");
	...
	/**
	 * Apply the federate outbound groups to the incoming groups on the message and
	 * return the intersection.
	 */
	public Set<String> filterFedOutboundGroups(List<String> fedOutboundGroups, NavigableSet<Group> clientInboundGroups,
			String fedId) {
		...
		if (!fedOutboundGroups.isEmpty() && !clientInboundGroups.isEmpty()) {
			...
			// apply the federate outbound groups, the matches with the client groups will
			// be preserved
	...
	public MissionPackageAnnounce createPackageAnnounceCot(ResourceDetails dt) {
		...
		final String baseUrl = CoreConfigFacade.getInstance().getRemoteConfiguration().getFederation()
				.getFederationServer().getWebBaseUrl();
		...
		if (!Strings.isNullOrEmpty(dt.getSenderUid())) {
			try {
				...
				MissionPackageDestinations mpDests = mapper.readValue(dt.getSenderUid(),
						MissionPackageDestinations.class);
		...
		String cot = "<?xml version='1.0' encoding='UTF-8' standalone='yes'?>" + "<event version='2.0' uid='" + uid
				+ "' type='b-f-t-r' time='" + time + "' start='" + time + "' stale='" + staleTime + "' how='h-e'>"
				+ "<point lat='0.0' lon='0.0' hae='9999999.0' ce='9999999' le='9999999' />" + "<detail>"
				+ "<fileshare sha256='" + shaHash + "' senderUid='" + uid + "' name='" + filename + "' filename='"
				+ filename + "' senderUrl='" + localMpUrl + "' sizeInBytes='" + sizeInBytes + "' senderCallsign='"
				+ callsign + "'/>" + "<marti>";
		...
		// + "<ackrequest uid='02629afa-ab2e-44a8-9048-2f517b72b221' ackrequested='true'
		// tag='MP-Grizzly' endpoint='192.168.1.9:4242:tcp'/>"
		// + "<precisionlocation geopointsrc='???' altsrc='???'/>"
		cot += "</marti>" + "</detail>" + "</event>";
	...
	public ROL generatePackageAnnounceROL(CotEventContainer toSend) {
		...
		/*
		 * <event version="2.0" uid="b695c532caba4b54953f8e2a4f0e5330" type="b-f-t-r"
		 * time="2017-09-22T18:13:27Z" start="2017-09-22T18:13:27Z"
		 * stale="2017-09-22T18:15:07Z" how="h-e"> <point lat="0.0" lon="0.0"
		 * hae="9999999.0" ce="9999999" le="9999999"/> <detail> <fileshare
		 * sha256="b695c532caba4b54953f8e2a4f0e5330"
		 * senderUid="b695c532caba4b54953f8e2a4f0e5330" name="a.pdf" filename="a.pdf"
		 * senderUrl=
		 * "https://127.0.0.1:8443/Marti/sync/content?hash=b695c532caba4b54953f8e2a4f0e5330"
		 * sizeInBytes="441359" senderCallsign="a.pdf"/><_flow-tags_
		 * marti1="2017-09-22T18:13:27Z"/></detail></event>
		 */
		...
		try {
			...
			details.setSenderCallsign(
					toSend.getDocument().selectSingleNode("//detail/fileshare/@senderCallsign").getStringValue());
			...
			details.setSize(Integer.parseInt(
					toSend.getDocument().selectSingleNode("//detail/fileshare/@sizeInBytes").getStringValue()));
			...
			try {
				...
				List<String> callsigns = (List<String>) toSend
						.getContextValue(StreamingEndpointRewriteFilter.EXPLICIT_CALLSIGN_KEY);
				@SuppressWarnings("unchecked")
				List<String> uids = (List<String>) toSend
						.getContextValue(StreamingEndpointRewriteFilter.EXPLICIT_UID_KEY);
		...
		try {
			...
			if (Strings.isNullOrEmpty(details.getSha256())) {
				throw new IllegalArgumentException(
						"sha 256 hash missing from package announcment CoT." + toSend.asXml());
			}
		...
		throw new IllegalStateException(
				"insufficient information to generate mission package announcmement ROL from CoT " + toSend.asXml());
	}
	...
	public void sendLatestContactsToFederate(FederateSubscription fedSubscription) {
		try {
			getLatestReachableContacts(fedSubscription).forEach((contact) -> {
				...
				List<String> srcGroups = groupManager.getGroups(fedSubscription.getUser()).stream()
						.filter(g -> g.getDirection() == Direction.OUT).map(g -> g.getName())
						.collect(Collectors.toList());
				
				FederatedEvent.Builder messageBuilder = FederatedEvent.newBuilder().setContact(contact)
						.addAllFederateGroups(srcGroups);

				if (fedSubscription instanceof FigFederateSubscription) {
					FigFederateSubscription figFederateSubscription = (FigFederateSubscription) fedSubscription;
					FederateGroupHopLimits groupHopsLimits = GroupFederationUtil.getInstance()
							.getFederateGroupHopsLimitsForFederate(figFederateSubscription.getFederate(), srcGroups);
					messageBuilder.setFederateGroupHopLimits(groupHopsLimits);
				}

				fedSubscription.submitLocalContact(messageBuilder.build(), System.currentTimeMillis());
			});
		} catch (Exception e) {
			if (logger.isDebugEnabled()) {
				...
				try {
					...
					if (sub != null) {
						...
						if (sub instanceof FederateSubscription) {
							...
							ContactListEntry contact = ContactListEntry.newBuilder().setUid(sub.clientUid)
									.setCallsign(sub.callsign).setOperation(CRUD.CREATE).build();
	...
	public NavigableSet<Group> addFederateGroupMapping(Multimap<String, String> inboundMap,
			List<String> federateGroups) {
	...
	public boolean isRemoteCASelfCA(String remoteCaFingerprint) {
		if (remoteCaFingerprint == null)
			return false;
		try {
			SSLConfig sslConfig = SSLConfig.getInstance(CoreConfigFacade.getInstance().getRemoteConfiguration()
					.getFederation().getFederationServer().getTls());
			...
			for (Enumeration<String> e = selfStore.aliases(); e.hasMoreElements();) {
				...
				String selfCaFingerprint = RemoteUtil.getInstance().getCertSHA256Fingerprint(selfCA);
				
				if (remoteCaFingerprint.equals(selfCaFingerprint)) {
					return true;
				}
			}
		} catch (Exception e) {
	...
	public FederateGroupHopLimits getFederateGroupHopsLimitsForFederate(Federate federate) {
		return getFederateGroupHopsLimitsForFederate(federate, federate.getOutboundGroup());
	}

	public FederateGroupHopLimits getFederateGroupHopsLimitsForFederate(Federate federate,
			Collection<String> groups) {
		List<FederateGroupHopLimit> federateGroupHopLimits = new ArrayList<>();

		Map<String, Integer> hopLimits = federate.getOutboundGroupHopLimit().stream()
				.collect(Collectors.toMap(x -> x.getGroupName(), x -> x.getHopLimit()));

		groups.forEach(group -> {
			if (hopLimits.containsKey(group)) {
				FederateGroupHopLimit federateGroupHopLimit = FederateGroupHopLimit.newBuilder().setGroupName(group)
						.setCurrentHops(0).setMaxHops(hopLimits.get(group)).build();

				federateGroupHopLimits.add(federateGroupHopLimit);
			}
		});

		FederateGroupHopLimits.Builder builder = FederateGroupHopLimits.newBuilder();
		builder.addAllLimits(federateGroupHopLimits);
		builder.setUseFederateGroupHopLimits(hopLimits.size() > 0);
