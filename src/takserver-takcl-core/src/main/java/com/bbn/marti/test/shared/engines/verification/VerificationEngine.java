...
//                            (UserState.VerificationPrepEngineUserStateInterface) client;
    ...
    public void missionAddResource(@NotNull AbstractUser user, @NotNull String missionName, @NotNull String dataUploadHash) {
        ...
        if (StateEngine.data.userHasMissionPermission(user, missionName, permission)) {
            ...
            Mission addResourceResponseMission = Assert.getSingleApiSetVerificationData(ReturnCodeSuccess, SampleObjects.Mission, rawResponse);

            client.stateEngineData = addResourceResponseMission;

            TreeSet<MissionContentDataContainer> contentsList = addResourceResponseMission.getContents();
            ...
            Mission validationMission = Assert.getSingleApiSetResponseData(ReturnCodeSuccess, SampleObjects.Mission, validationResponse);

            HashMap<Pattern, Object> exceptions = null;
            if (TestExceptions.MISSION_IGNORE_GROUPS_MISSING_IN_ADD_RESOURCE_RESPONSE || TestExceptions.MISSION_IGNORE_ADD_RESOURCE_RESPONSE_MISSING_UID_OBJECTS) {
                ...
                if (TestExceptions.MISSION_IGNORE_ADD_RESOURCE_RESPONSE_MISSING_UID_OBJECTS) {
                    exceptions.put(MISSION_UIDS_PATTERN, new TreeSet<>());
                }
                if (TestExceptions.MISSION_IGNORE_ADD_RESOURCE_RESPONSE_MISSING_UID_DETAILS) {
                    exceptions.put(MISSION_UID_DETAILS_PATTERN, EXCEPTION_IGNORE_VALUE);
                }
            }
            Assert.assertTrue("The retrieved mission is different than the returned mission!", addResourceResponseMission.assertMatchesExpectation(validationMission, exceptions));
